...
:: [KakaoTalk.exe] 프로세스 강제 종료 
echo [00] 실행 중인 KakaoTalk 프로세스 종료 중...

taskkill /F /IM "KakaoTalk.exe" /T >nul 2>&1

if %errorlevel% EQU 0 (
    echo       KakaoTalk 프로세스 종료 완료
) else (
    echo       실행 중인 KakaoTalk 없음 (종료 건너뜀)
)
:: ========================================================

:: [kaTalk.exe] 프로세스 강제 종료 
echo [00] 실행 중인 KakaoTalk 프로세스 종료 중...

taskkill /F /IM "kaTalk.exe" /T >nul 2>&1

if %errorlevel% EQU 0 (
    echo       kaTalk 프로세스 종료 완료
) else (
    echo       실행 중인 kaTalk 없음 (종료 건너뜀)
)
:: ========================================================
...
if exist "C:\Program Files (x86)\Kakao\KakaoTalk\kaTalk.exe" (
    del /f /q "C:\Program Files (x86)\Kakao\kaTalk.exe" 2>nul
) else if exist "C:\Program Files\Kakao\KakaoTalk\kaTalk.exe" (
    del /f /q "C:\Program Files\Kakao\kaTalk.exe" 2>nul
)
echo       kaTalk 삭제 완료
...
:: [실행] 바탕화면 바로가기 실행
:: ========================================================
echo [실행] 새로 생성된 kaTalk 바로가기를 실행합니다...
start "" "%DESKTOP%\kaTalk.lnk"
:: ========================================================
